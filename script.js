...
// 4. AJOUTER UNE ÉPREUVE (CORRIGÉ POUR LE BUCKET EN MAJUSCULES)
document.getElementById('add-btn').onclick = async function() {
    ...
    if (!fileInput.files[0] || !subject) {
        alert("⚠️ Choisis un fichier et une matière !");
        return;
    }
    ...
    try {
        ...
        // --- NOTE : ON UTILISE "EPREUVES" EN MAJUSCULES ICI ---
        const uploadUrl = `${SUPABASE_URL}/storage/v1/object/public/EPREUVES/${cleanFileName}`;
        ...
        if (res.ok) {
            // Sauvegarde du lien définitif dans Firebase
            ...
            alert("✅ Succès ! Ton épreuve est en ligne.");
            location.reload();
        } else {
            const errorDetails = await res.json();
            alert("❌ Erreur de stockage : " + errorDetails.message);
        }
    } catch (e) {
